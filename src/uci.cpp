...
static Position* uciPos = nullptr; // The 'position' state for the UCI engine.
static StateListPtr* uciStates = nullptr;
void UCI::new_game() {
	// Reset uciPos.
	if (uciPos != nullptr)
		delete uciPos;
	...
	// Reset uciStates.
    if (uciStates != nullptr)
		delete uciStates;
	uciStates = new StateListPtr(new std::deque<StateInfo>(1));
	
	// Set uciPos to new game state.
